...
Adotada pelo `tech-ai` desde a primeira fatia executável. A organização segue a referência do
...
- o `tech-ai` depende do contrato publicado pelo `tech-ingestao`, nunca do pacote Python dele.
...
`tech-ai` decide como os dados aprovados são apresentados ao modelo: instrução de sistema,
